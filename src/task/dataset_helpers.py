...
def load_data(dataset: str, num_ex: int = -1, split: str = 'train', num_users: int = 1):
    """
...
class OurInputExample:
    ...
    def __str__(self):
        return f"<InputExample> article ID {self.id} (User: {self.user_id}): {self.article[:100]}..."
...
class OurInputDataset(Dataset): 
    ...
    *_keys are string keys in each example dictionary in the given data loaded from hugging face
    """
    def __init__(self, data, num_ex: int, id_key: str, article_key: str, doc_type: str = '', num_users: int = 1):
        from itertools import islice
        if num_ex > 0:
            data = list(islice(data, num_ex))
        
        # Use predefined user IDs
        user_ids = [f"user{i+1}" for i in range(num_users)]
        self.dataset = []
        
        for d in data:
            # Randomly assign a user ID to each example
            user_id = random.choice(user_ids)
            user_pref = USER_PREFERENCES[user_id].preferences.get(doc_type, "")
    ...
    def get_examples_by_user(self, user_id: str) -> List[OurInputExample]:
        ...
        return [ex for ex in self.dataset if ex.user_id == user_id]
    
    def get_unique_users(self) -> List[str]:
        """Get list of unique user IDs in the dataset"""
        return list(set(ex.user_id for ex in self.dataset))
